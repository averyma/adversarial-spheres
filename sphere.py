...
def train_adv(param, dim, r, total_samples, batch_size, err_freq, model, opt, device):
    ...
    with trange(num_itr) as t:
        for i in range(num_itr):
            ...
            t.set_postfix(loss = loss.item(),
                          acc = "{0:.2f}%".format(batch_acc),
                          good_alpha = "nan" if stats["alpha"].size ==0 
                                             else "{0:.2f}%".format(stats["alpha"][-1,0]))
            t.update()
            ...
            if (i+1) % err_freq == 0:
                ...
                ana_err = analytic_err(model, r)
                alpha_stats = get_alpha_stats(model, r)
                stats = save_stats([loss.item(), test_loss], [batch_acc, test_acc],
                                    ana_err, alpha_stats, i+1, stats)
                if np.abs( alpha_stats[0] - 100.) < 1e-5:
                    break
    return stats
